$(document).ready(function () {
    ...
    $("#deleteAllApplicationNumber").on('submit', (function (e) {
        e.preventDefault();
        ...
        if (!length) {
            ...
            $("#messageBox").html('<p>No record selected please select application number.</p>');
            return false;
        }

        $.ajax({
            url: '/admin/application-number/destroyAll',
            type: 'post',
            ...
            success: function (result) {
                ...
                if (result.status) {
                    ...
                    $("#messageModal").modal('show');
                    $("#messageBox").html('<p>Application Number information  deleted successfully</p>');
                }
    ...
    var table = $('#applicationNumberTable').DataTable({
        rowReorder: true,
        stateSave: true,
    ...
    table.on('row-reorder', function (e, diff, edit) {
        ...
        $.ajax({
            type: 'POST',
            url: '/admin/application-number/updateSortorder',
            data: {
    ...
    $('.status').change(function () {
        ...
        $.ajax({
            ...
            url: '/admin/application-number/updateStatus/',
            dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
...
function deleteRecord(id, title, message) {
    ...
    $("#deleteAllModalButton").on("click", function () {
        ...
        $.ajax({
            ...
            url: '/admin/application-number/destroy/',
            dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
