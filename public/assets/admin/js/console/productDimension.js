$(document).ready(function () {
    ...
    $("#deleteAllProductDimension").on('submit', (function (e) {
        e.preventDefault();
        ...
        if (!length) {
            ...
            $("#messageBox").html('<p>No record selected please select Product Dimension.</p>');
            return false;
        }

        $.ajax({
            url: '/admin/product-dimension/destroyAll',
            type: 'post',
            ...
            success: function (result) {
                ...
                if (result.status) {
                    ...
                    $("#messageModal").modal('show');
                    $("#messageBox").html('<p>Product Dimension information  deleted successfully</p>');
                }
    ...
    var table = $('#productDimensionTable').DataTable({
        rowReorder: true,
        stateSave: true,
    ...
    table.on('row-reorder', function (e, diff, edit) {
        ...
        $.ajax({
            type: 'POST',
            url: '/admin/product-dimension/updateSortorder',
            data: {
    ...
    $('.status').change(function () {
        ...
        $.ajax({
            ...
            url: '/admin/product-dimension/updateStatus/',
            dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
...
function deleteRecord(id, title, message) {
    ...
    $("#deleteAllModalButton").on("click", function () {
        ...
        $.ajax({
            ...
            url: '/admin/product-dimension/destroy/',
            dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
