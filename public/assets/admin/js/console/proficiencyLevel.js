$(document).ready(function () { 
    ...
    $("#deleteAllProficiencyLevel").on('submit', (function (e) {
        e.preventDefault();
        ...
        if (!length) {
            ...
            $("#messageBox").html('<p>No record selected please select proficiency level.</p>');
            return false;
        }

        $.ajax({
            url: '/admin/proficiencyLevel/destroyAll',
            type: 'post',
            ...
            success: function (result) {
                ...
                if (result.status) {
                    ...
                    $("#messageModal").modal('show');
                    $("#messageBox").html('<p>Proficiency level information  deleted successfully</p>');
                }
    ...
    var table = $('#proficiencyLevelTable').DataTable({
        rowReorder: true,
        stateSave: true,
    ...
    table.on('row-reorder', function (e, diff, edit) {
        ...
        $.ajax({
            type: 'POST',
            url: '/admin/proficiencyLevel/updateSortorder',
            data: {
    ...
    $('.status').change(function () {
        ...
        $.ajax({
            ...
            url: '/admin/proficiencyLevel/updateStatus',
            dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
...
function deleteRecord(id, title, message) {
    ...
    $("#deleteAllModalButton").on("click", function () {
        ...
        $.ajax({
            ...
            url: '/admin/proficiencyLevel/destroy',
            dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
